...
const transfer_reclaim = async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < used.length; i++) {
            giving.push({Given: used[i].Given, Location: Location, Item_id: used[i].Item_id})
            taking.push({Quantity: used[i].Quantity - used[i].Given, ItemLocation_id: used[i].FKItemLocation})
        }

        for (let i = 0; i < used.length; i++) {
            const give = `UPDATE public.itemlocation SET "Quantity" = itemlocation."Quantity" + ${giving[i].Given} WHERE "Location_id" = ${giving[i].Location} AND "Item_id" = ${giving[i].Item_id}`
